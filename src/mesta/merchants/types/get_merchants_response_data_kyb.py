
import datetime as dt
import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata
from .get_merchants_response_data_kyb_status import GetMerchantsResponseDataKybStatus


class GetMerchantsResponseDataKyb(UniversalBaseModel):
    """
    Know Your Business verification information
    """

    status: typing.Optional[GetMerchantsResponseDataKybStatus] = pydantic.Field(default=None)
    """
    KYB verification status
    """

    status_updated_at: typing_extensions.Annotated[
        typing.Optional[dt.datetime],
        FieldMetadata(alias="statusUpdatedAt"),
        pydantic.Field(alias="statusUpdatedAt", description="Timestamp of the last KYB status update"),
    ] = None
    """
    Timestamp of the last KYB status update
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
